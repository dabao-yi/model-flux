#!/usr/bin/env bash
# model-flux smoke test
# Usage: ./scripts/smoke.sh [base_url]
#   defaults to http://localhost:19090
#
# Verifies:
#   1. /health returns ok
...
pick_default_model() {
  ...
  local aliases first_alias mi ds
  aliases="$(read_env_var MODEL_ALIASES)"
  first_alias="$(printf '%s' "$aliases" | tr ',' '\n' | head -1 | sed 's/[[:space:]]*=.*//')"
  if [[ -n "$first_alias" ]]; then
    MODEL="$first_alias"
    return
  fi
  mi="$(read_env_var MIMO_MODELS | tr ',' '\n' | head -1)"
  if [[ -n "$mi" ]]; then
    MODEL="$mi"
    return
  fi
  ds="$(read_env_var DEEPSEEK_MODELS | tr ',' '\n' | head -1)"
  if [[ -n "$ds" ]]; then
    MODEL="$ds"
    return
  fi
...
# Upstream MiMo/DeepSeek may intermittently return 402 (balance/rate); retry a few times.
post_status_upstream() {
  local path="$1"
  local body="$2"
  local code attempt
  for ((attempt = 1; attempt <= SMOKE_RETRY; attempt++)); do
    code="$(post_status "$path" "$body")"
...
echo "model-flux smoke @ $BASE (model=$MODEL, auth=$([[ -n "$AUTH_KEY" ]] && echo on || echo off))"
echo
...
check "string input"              "200" "$(post_status_upstream /v1/responses "{\"model\":\"$MODEL\",\"input\":\"reply pong\"}")"
check "array {role,content}"      "200" "$(post_status_upstream /v1/responses "{\"model\":\"$MODEL\",\"stream\":true,\"input\":[{\"role\":\"user\",\"content\":\"reply pong\"}]}")"
check "array message+input_text"  "200" "$(post_status_upstream /v1/responses "{\"model\":\"$MODEL\",\"stream\":true,\"input\":[{\"type\":\"message\",\"role\":\"user\",\"content\":[{\"type\":\"input_text\",\"text\":\"reply pong\"}]}]}")"
...
SSE_DONE_COUNT=0
for ((attempt = 1; attempt <= SMOKE_RETRY; attempt++)); do
  SSE_TMP="$(mktemp -t model-flux-sse.XXXXXX)"
...
for eff in none minimal low medium high xhigh; do
  check "deepseek effort=$eff"  "200" "$(post_status_upstream /v1/responses "$(EFFORT_BODY "$DS_MODEL" "$eff")")"
done
# MiMo path: same — but `xhigh` must be clamped to `high` by the proxy or MiMo 400s.
for eff in none minimal low medium high xhigh; do
  check "mimo effort=$eff"      "200" "$(post_status_upstream /v1/responses "$(EFFORT_BODY "$MI_MODEL" "$eff")")"
done
...
TOOL_RT_TMP="$(mktemp -t model-flux-rt.XXXXXX)"
RT_CALL_ID=""
for ((attempt = 1; attempt <= SMOKE_RETRY; attempt++)); do
  ...
  RT_CALL_ID="$(python3 -c 'import sys,json; d=json.load(open(sys.argv[1])); print(next((o["call_id"] for o in d.get("output",[]) if o.get("type")=="function_call"), ""))' "$TOOL_RT_TMP" 2>/dev/null || true)"
  [[ -n "$RT_CALL_ID" ]] && break
  sleep 2
done
if [[ -n "$RT_CALL_ID" ]]; then
  if has_env_key DEEPSEEK_API_KEY || has_env_key DEEPSEEK_API_KEYS; then
    ...
    skip "deepseek tool-roundtrip (cache HIT, thinking preserved)" "no DEEPSEEK_API_KEY configured"
  fi
else
  skip "deepseek tool-roundtrip (cache HIT, thinking preserved)" "model did not return function_call"
fi
rm -f "$TOOL_RT_TMP"
...
check "deepseek tool-roundtrip (cache MISS, safety-net)" "200" "$(post_status_upstream /v1/responses "$TOOL_RT_MISS_BODY")"
